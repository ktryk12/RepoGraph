...
def _run_kafka_mode() -> int:
    ...
    # Voice I/O — starter sidst, kun hvis VOICE_ENABLED=true
...
def _start_security_runtime(redis_client: Any) -> SecurityRuntime | None:
    if redis_client is None:
        return None
    sqlite_path = os.getenv("SQLITE_PATH", "state/security_events.sqlite")
...
def _ensure_model_ready() -> None:
    """POST /models/gemma-26b/ensure-ready on startup. Non-fatal on failure."""
    base_url = str(os.getenv("MODEL_MANAGER_BASE_URL", "http://localhost:8112")).rstrip("/")
    model_id = str(os.getenv("STARTUP_MODEL_ID", "gemma-26b"))
    timeout = float(os.getenv("MODEL_MANAGER_STARTUP_TIMEOUT", "30"))
    from urllib.request import Request, urlopen
    from urllib.error import URLError
    endpoint = f"{base_url}/models/{model_id}/ensure-ready"
    try:
        req = Request(
            url=endpoint,
            data=b"{}",
        ...
        with urlopen(req, timeout=timeout) as resp:
            status = resp.getcode()
        logger.info("model_ensure_ready model=%s status=%s", model_id, status)
    except Exception as exc:
        logger.warning("model_ensure_ready_failed model=%s error=%s — continuing", model_id, exc)
