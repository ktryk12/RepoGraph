#!/usr/bin/env python3
"""
Voice Service Database Migration Script

This script initializes the voice-service database and creates all required tables.
...
def main():
    """Run database migration for voice-service"""
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
    )

    logger = logging.getLogger("voice-service-migration")

    try:
        logger.info(f"Starting voice-service database migration (schema version: {SCHEMA_VERSION})")
