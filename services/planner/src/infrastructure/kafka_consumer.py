...
def _require_consumer() -> Any:
    if Consumer is None:
        raise ImportError("confluent-kafka is required for planner consumer")
    return Consumer


class KafkaPlannerConsumer:
    def __init__(
        self,
        *,
        ...
        self._ready_topic = str(ready_topic).strip() or "decision.truthpack.ready"
        ...
        self._consumer.subscribe([self._intent_topic, self._ready_topic])
        log_milestone(
            logger,
            "consumer_subscribe",
            service_name=_SERVICE_NAME,
            component=_COMPONENT,
            topic=",".join([self._intent_topic, self._ready_topic]),
            decision_id="",
            context_id="",
            episode_id="",
            event_type="",
            event_id="",
            trace_id="",
            topics=[self._intent_topic, self._ready_topic],
        )
    ...
    def run_once(self) -> int:
        ...
        if topic == self._intent_topic:
            ...
            self._service.handle_ready(payload)
