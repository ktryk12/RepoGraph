...
def _run_aesa_suite(
    *,
    ...
    env = os.environ.copy()
    if model_path:
        env["AESA_REPAIR_POLICY_MODEL"] = model_path
    else:
        env.pop("AESA_REPAIR_POLICY_MODEL", None)
...
def _evaluate(
    *,
    ...
    if avg_rep_increase > max_increase_avg_repairs:
        reasons.append(
            "avg_repairs increased by "
            f"{avg_rep_increase:.4f} (base={baseline['avg_repairs']:.4f}, cand={candidate['avg_repairs']:.4f})"
        )
    ...
    if candidate["scope_violations"] > baseline["scope_violations"]:
        reasons.append(
            "scope_violations increased "
            f"(base={baseline['scope_violations']:.0f}, cand={candidate['scope_violations']:.0f})"
        )
...
def _load_candidate_metadata(candidate_model: str) -> Dict[str, Any]:
    ...
    candidates = [p.with_suffix(".meta.json"), p.parent / "metadata.json"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="A/B trials for repair policy model.")
    parser.add_argument(
        "--candidate-model",
        default=None,
        help="Candidate model path (.onnx or .npz). Default: AESA_REPAIR_POLICY_MODEL env.",
    )
    ...
    parser.add_argument(
        "--train-jsonl",
        default=None,
        help="Training JSONL used to build candidate model. Used for leakage guard.",
    )
    parser.add_argument(
        "--strict-no-leakage",
        action="store_true",
        help="Fail if training IDs overlap with eval task IDs.",
    )
    parser.add_argument("--out-dir", default="artifacts/repair_policy_trials", help="Output directory.")
    parser.add_argument("--seed", type=int, default=42, help="Determinism seed recorded in artifacts.")
    parser.add_argument(
        "--max-drop-pass-rate",
        type=float,
        default=0.0,
        help="Allowable pass-rate drop before failing.",
    )
    parser.add_argument(
        "--max-increase-avg-repairs",
        type=float,
        default=0.0,
        help="Allowable avg_repairs increase before failing.",
    )
    args = parser.parse_args()

    candidate_model = args.candidate_model or os.getenv("AESA_REPAIR_POLICY_MODEL")
    if not candidate_model:
        raise SystemExit("candidate model path required via --candidate-model or AESA_REPAIR_POLICY_MODEL")
    ...
    if args.strict_no_leakage:
        ...
        if overlap:
            raise SystemExit(
                f"data leakage detected: {len(overlap)} overlapping ids between train/eval (sample={overlap[:5]})"
            )
    ...
    candidate_meta = _load_candidate_metadata(candidate_model)
    candidate_model_hash = _sha256_file(candidate_model)

    summary = {
        "accepted": bool(ok),
        "candidate_model": candidate_model,
        "candidate_model_hash": candidate_model_hash,
        "candidate_model_version": candidate_meta.get("model_version"),
        "candidate_feature_dim": candidate_meta.get("feature_dim"),
        "candidate_feature_schema_version": candidate_meta.get("feature_schema_version"),
    ...
    trial_md_path.write_text(
        (
            ...
            f"- candidate_model_hash: {candidate_model_hash}\n"
            ...
            f"- pass_rate: {baseline_metrics['pass_rate']:.4f} -> {candidate_metrics['pass_rate']:.4f} "
            f"(delta {deltas['pass_rate']:+.4f})\n"
            f"- avg_repairs: {baseline_metrics['avg_repairs']:.4f} -> {candidate_metrics['avg_repairs']:.4f} "
            f"(delta {deltas['avg_repairs']:+.4f})\n"
            f"- timeouts: {baseline_metrics['timeouts']:.0f} -> {candidate_metrics['timeouts']:.0f} "
            f"(delta {deltas['timeouts']:+.0f})\n"
            f"- scope_violations: {baseline_metrics['scope_violations']:.0f} -> {candidate_metrics['scope_violations']:.0f} "
            f"(delta {deltas['scope_violations']:+.0f})\n"
            f"- reasons: {', '.join(reasons) if reasons else '(none)'}\n"
        ),
        encoding="utf-8",
    )
    print(f"[repair-policy-trial] {json.dumps(summary, ensure_ascii=True)}")
