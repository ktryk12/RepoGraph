...
from aesa.experts.router.router_features import FEATURE_SCHEMA_VERSION
...
def export_onnx(*, npz_path: str | Path, onnx_path: str | Path) -> None:
    ...
    graph = helper.make_graph(nodes=nodes, name="router_policy_ranker", inputs=[x], outputs=[y], initializer=inits)
    model = helper.make_model(graph, producer_name="babyai.router_policy")
    model.opset_import[0].version = 17
    model.metadata_props.extend(
        [
            onnx.StringStringEntryProto(key="model_version", value=str(npz_meta.get("version", "router_policy_v1"))),
            onnx.StringStringEntryProto(key="feature_dim", value=str(input_dim)),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export router checkpoint (.npz) to ONNX.")
    ...
    print(f"[router-export] onnx={args.out}")
    return 0
