...
from aesa.experts.router.router_features import FEATURE_SCHEMA_VERSION, INPUT_DIM
from aesa.experts.router.router_model import RouterPolicyMiniModel, build_examples_from_router_logs
...
from scripts.export_router_onnx import export_onnx


def main() -> int:
    parser = argparse.ArgumentParser(description="Train router mini-model from routing JSONL.")
    parser.add_argument("--jsonl", default="eval/aesa/router_train.jsonl", help="Input routing JSONL path.")
    parser.add_argument("--out", default="models/router_policy_v1/model.npz", help="Output model path (.npz).")
    ...
    parser.add_argument("--onnx-out", default=None, help="Optional ONNX output path.")
    ...
    try:
        get_killswitch_service().require_write(
            operation="scripts.train_router_model",
            scope="TRAIN_WRITE",
        )
    except KillSwitchViolation as exc:
        print(f"[router-policy] killswitch violation: {exc}")
        return 2

    try:
        constitution.require("training_dataset", {"dataset_path": args.jsonl})
    except Exception as exc:
        print(f"[router-policy] constitution violation: {exc}")
        return 2
    ...
    try:
        ...
        print(f"[router-policy] training judge gate violation: {exc}")
        return 2
    except Exception as exc:
        print(f"[router-policy] curation/gate error: {exc}")
        return 2

    constitution.require("write_path", {"path": curated_input_path})
    write_curated_jsonl(curated_input_path, curation.curated_rows)
    examples = build_examples_from_router_logs(curated_input_path)
    if not examples:
        print(f"[router-policy] no training examples found in {args.jsonl}")
        return 2

    model = RouterPolicyMiniModel(
    ...
    npz_meta_path.write_text(json.dumps(metadata, indent=2, ensure_ascii=True), encoding="utf-8")
    if onnx_exported:
        onnx_out.with_suffix(".meta.json").write_text(
            json.dumps(metadata, indent=2, ensure_ascii=True),
            encoding="utf-8",
        )

    print(f"[router-policy] trained examples={len(examples)} loss={stats.get('loss', 0.0):.4f}")
    print(f"[router-policy] model={out_path}")
    if onnx_exported:
        print(f"[router-policy] onnx={onnx_out}")
    elif onnx_error:
        print(f"[router-policy] onnx_export_skipped error={onnx_error}")
        return 2
    print(f"[router-policy] metadata={metadata_path}")
    return 0
