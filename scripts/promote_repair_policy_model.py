...
def main() -> int:
    parser = argparse.ArgumentParser(description="Promote candidate repair policy model to blessed path.")
    parser.add_argument("--trial", default="artifacts/repair_policy_trials/trial.json", help="Trial summary JSON path.")
    parser.add_argument("--candidate-model", default=None, help="Override candidate model path.")
    parser.add_argument("--target-model", default="models/repair_policy_v1/model.onnx", help="Blessed model output path.")
    parser.add_argument("--latest-file", default="models/repair_policy_v1/LATEST", help="LATEST marker file.")
    parser.add_argument("--promotions-log", default="artifacts/repair_policy_trials/promotions.jsonl", help="Promotion log JSONL.")
    ...
    try:
        get_killswitch_service().require_write(
            operation="scripts.promote_repair_policy_model",
            scope="PROMOTE_ACTIVE",
        )
    except KillSwitchViolation as exc:
        print(f"[promote-repair-policy] killswitch violation: {exc}")
        return 2
    ...
    policy = load_promotion_policy(path=policy_path, model_family="repair_policy_v1")
    report = get_promotion_gate_service().evaluate(
        trial=trial,
        policy=policy,
        model_family="repair_policy_v1",
...
def _copy_parent_metadata(src: Path, dst: Path) -> None:
    ...
    if not src_meta.exists():
        return
    dst_meta = dst.parent / "metadata.json"
    get_constitution_service().require("write_path", {"path": dst_meta})
